...
///
/// We mirror the cheap structural checks here so we can drop bad quotes before
/// committing payment. Note: this does NOT call `verify_quote_signature` or
/// `verify_quote_content` — those mirrors are valuable but live behind ML-DSA
/// verification (~1 ms) and are deliberately deferred to a follow-up so this
/// patch stays focused on the Apr 30 production failure shape (BLAKE3 binding).
...
impl Client {
    ...
    pub async fn get_store_quotes(
        ...
        // Track bad-binding peers separately so we can surface their count in
        // diagnostics — they're a special class of failure (peer misconfigured,
        // not network-broken) and the user benefits from seeing them called out.
        let mut bad_binding_count = 0usize;
        ...
        if quotes.len() >= CLOSE_GROUP_SIZE {
            ...
            info!(
                "Collected {} quotes for address {} ({total_responses} responses: \
                 ...
                 {bad_binding_count} bad_bindings)",
                quotes.len(),
                hex::encode(address),
            );
            return Ok(quotes);
        ...
        Err(Error::InsufficientPeers(format!(
            "Got {quote_count} quotes, need {CLOSE_GROUP_SIZE} ({total_responses} responses: \
             ...
             {bad_binding_count} with mismatched peer bindings). Failures: [{}]",
            failures.join("; ")
        )))
    }
}
...
mod tests {
    ...
    //! same key material that ships on the wire. This avoids the
    //! "synthetic-key" honesty problem: the tests exercise exactly the same
    //! `compute_address(pub_key)` byte path that runs in production. The
    //! "bad" quote is built by **swapping** the public key field with a
    //! different real keypair's public key — the exact shape produced by the
    //! Apr 30 production failure (an operator running two co-located
    ...
    /// signed with peer B's key (or peer B's key is inserted into A's quote).
    fn bad_quote_real() -> (PeerId, Vec<MultiAddr>, PaymentQuote, Amount) {
        let claimed = gen_keypair();
        let signing = gen_keypair();
        // Distinct keypairs ⇒ distinct BLAKE3 outputs ⇒ a real binding mismatch.
    ...
    /// Independent re-implementation of the storer-side binding check
    /// (`ant-node/src/payment/verifier.rs::validate_peer_bindings`). Used to
    /// prove that quotes the client retains will not be rejected by the
    /// storer — and that quotes the client drops would have been rejected.
    ///
    /// This deliberately re-derives the check from spec rather than calling
    /// `quote_binding_is_valid` so the tests are not "function == itself".
    fn storer_would_accept(peer_id: &PeerId, quote: &PaymentQuote) -> bool {
        // Storer first checks the pub_key parses as ML-DSA-65 (length 1952).
        if MlDsaPublicKey::from_bytes(&quote.pub_key).is_err() {
            return false;
        }
        // Then checks BLAKE3(pub_key) == peer_id bytes.
        compute_address(&quote.pub_key) == *peer_id.as_bytes()
    }
    ...
    fn binding_accepts_real_self_consistent_keypair() {
        ...
        // Cross-check against the independent storer-spec implementation.
    ...
    fn filter_drops_only_bad_bindings_and_leaves_storer_acceptable_quotes() {
        ...
        // a storer running the spec check. This is what guarantees the
        // ProofOfPayment we eventually build is not pre-rejected.
        for (peer_id, _, quote, _) in &quotes {
            assert!(
                storer_would_accept(peer_id, quote),
                "every retained quote must satisfy the storer-side binding spec"
            );
        }
    }
    ...
    fn repro_apr_30_storer_would_have_rejected_pre_filter_and_accepts_post_filter() {
        ...
        let storer_would_reject_count = quotes
            .iter()
            .filter(|(p, _, q, _)| !storer_would_accept(p, q))
            .count();
        assert_eq!(
        ...
        // Step 3: prove the storer would accept every survivor.
        for (peer_id, _, quote, _) in &quotes {
            assert!(
                storer_would_accept(peer_id, quote),
